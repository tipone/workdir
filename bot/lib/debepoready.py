...
class Builder:
	...
	def _factory(self):
		...
		self.factory.addStep(steps.ShellCommand(command=["bash", "debrepo-ready.sh"], workdir=self.scripts))
		self.factory.addStep(trigger.Trigger(schedulerNames=['storboxdebtest-triggered'], waitForFinish=True))
	def build(self):
		self._factory()
