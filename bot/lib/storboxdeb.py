...
class Builder:
	...
	def _factory(self):
		...
		self.factory.addStep(steps.Git(repourl=self.repourl, branch=self.branch, mode='full', method='clobber', submodules=True, workdir=self.gitdir))		
		self.factory.addStep(steps.ShellCommand(command=["bash", "storboxdeb.sh"], workdir=self.scripts))
#		self.factory.addStep(trigger.Trigger(schedulerNames=['debrepoready-triggered'], waitForFinish=True))
		self.factory.addStep(trigger.Trigger(schedulerNames=['stortest1-triggered'], waitForFinish=True))
	def build(self):
		self._factory()
